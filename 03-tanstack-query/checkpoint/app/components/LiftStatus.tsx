...
const fetchLifts = async (): Promise<Lift[]> => {
  console.log('fetchLifts called')
  try {
    const response = await fetch('/api/lifts')
    console.log('fetchLifts response:', response.status, response.ok)
    if (!response.ok) {
      throw new Error(`Failed to fetch lift data: ${response.status}`)
    }
    const data = await response.json()
    console.log('fetchLifts data:', data?.length, 'lifts')
    return data
  } catch (error) {
    console.error('fetchLifts error:', error)
    throw error
  }
}
...
export function LiftStatus() {
  ...
  console.log('LiftStatus render:', { isLoading, isError, error, dataLength: lifts?.length })
